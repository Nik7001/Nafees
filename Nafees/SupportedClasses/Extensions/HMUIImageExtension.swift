...
extension UIImage {
    ...
    func resizedTo1MB() -> UIImage? {
        ...
        var imageSizeKB = Double(imageData.count) / 2048.0 // ! Or devide for 1024 if you need KB but not kB
        
        while imageSizeKB > 2048 { // ! Or use 1024 if you need KB but not kB
            guard let resizedImage = resizingImage.resized(withPercentage: 0.9),
                let imageData = resizedImage.pngData()
                else { return nil }
            
            resizingImage = resizedImage
            imageSizeKB = Double(imageData.count) / 2048.0 // ! Or devide for 1024 if you need KB but not kB
        }
    ...
    func compressTo(_ expectedSizeInMb:Int) -> UIImage? {
        let sizeInBytes = expectedSizeInMb * 2048 * 2048
